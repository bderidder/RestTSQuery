...
/**
 * This class was NOT written by Bavo De Ridder! See below for the name 
 * of the author and pointers to the website.
 * 
 * Change Log by Bavo De Ridder (http://www.coderspotting.org/)
 *
 * Replaced the use of "Vector" with types "List". Replaced all
 * "DEBUG/printStackTrace" call with a Logger/Level.SEVERE
 */
/**
 * JTS3ServerQuery library version 1.0.9
 * <br><br>
 * This library allows you to create a query connection to the Teamspeak 3
 * telnet interface. Almost anything is supported: Query lists or just
 * informations, get log entries, receiving events, add or delete complains,
 * kick or move clients and of course send own commands.
 * <br><br>
 * This library is free for use, but please notify me if you found a bug or if
 * you have some suggestion. The author of this library is not responsible for
 * any damage or data loss! It is not allowed to sell this library for money, it
 * has to be free to get!<br><br>
 *
 * <b>E-Mail:</b><br><a
 * href="mailto:info@stefan1200.de">info@stefan1200.de</a><br><br>
 * <b>Homepage:</b><br><a href="http://www.stefan1200.de"
 * target="_blank">http://www.stefan1200.de</a>
 *
...
public class JTS3ServerQuery
{
    /**
     * Setting DEBUG to
     * <code>true</code> will do a printStackTrace() on every exception and
     * write the communication log file. It is also possible to set the filename
     * and path to the communication log file, see DEBUG_COMMLOG_PATH.
     *
     * @since 1.0.4
     ...
     * <code>null</code> to disable writing of this log file. This will be used
     * only if DEBUG is set to
     * <code>true</code>. In the communication log file you can see any outgoing
     * and incoming messages between this library and the TS3 server.
     *
     * @since 1.0.4
    ...
    /**
     * List mode for getList(), use this mode to get a list of clients currently
     * online.<br><br>
    ...
    /**
     * List mode for getList(), use this mode to get a list of current
     * channels.<br><br>
     * Possible optional arguments:<br>
    ...
    /**
     * List mode for getList(), use this mode to get a list of virtual
     * servers.<br><br>
     * Possible optional arguments:<br>
    ...
    /**
     * List mode for getList(), use this mode to get a list of all clients in
     * database. By default only the first 25 entries will be returned.<br><br>
     * Possible optional arguments:<br>
     ...
     * Without arguments you get a list of all complains on the server. Use the
     * optional argument to get complains of only one client.<br><br>
     * Possible optional arguments:<br>
    ...
    /**
     * Info mode for getInfo(), use this mode to get informations about the
    ...
    /**
     * Info mode for getInfo(), use this mode to get informations about a
     * channel.
     *
     * @see JTS3ServerQuery#getInfo(int, int)
     */
    ...
    /**
     * Permission list mode for getPermissionList(), use this mode to get a list
    ...
    /**
     * Permission list mode for getPermissionList(), use this mode to get a list
     * of client permissions.
     *
     * @see JTS3ServerQuery#getPermissionList(int, int)
     */
    public static final int PERMLISTMODE_CLIENT = 23;
    /**
     * Text message target mode for sendTextMessage() to send a message to a
     * single client.
     *
    ...
    /**
     * Text message target mode for sendTextMessage() to send a message to all
     * virtual servers.
     *
     * @see JTS3ServerQuery#sendTextMessage(int, int, String)
     */
    ...
    /**
     * Event mode for addEventNotify() to add server chat events (like receiving
    ...
    /**
     * Event mode for addEventNotify() to add channel chat events (like
     * receiving or sending chat messages).
     *
     * @since 0.7
     * @see JTS3ServerQuery#addEventNotify(int, int)
     */
    ...
    /**
     * Event mode for addEventNotify() to add private chat events (like
     * receiving or sending chat messages).
     *
     * @since 0.7
     * @see JTS3ServerQuery#addEventNotify(int, int)
    ...
    /**
     * Event mode for addEventNotify() to add server events (like clients join
     * or left the server).
     *
     * @since 0.7
     * @see JTS3ServerQuery#addEventNotify(int, int)
    ...
    /**
     * Event mode for addEventNotify() to add channel events (like clients join
     * or left the channel).<br><br>
     * <b>Notice:</b><br>
    ...
    /**
     * New JTS3ServerQuery object which allows to set a prefix for the
     * handleAction thread name.
     *
    ...
    private synchronized void writeCommLog(String commMessage)
    {
        ...
        {
            Logger.getLogger(JTS3ServerQuery.class.getName()).log(Level.SEVERE, null, e);
        }
    }
    ...
    private void eventNotifyRun()
    {
        if (eventNotifyCheckActive && isConnected())
        {
            ...
            {
                Logger.getLogger(JTS3ServerQuery.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }

    /**
     * Set a class that should receive the Teamspeak events. This class must
     * implement the TeamspeakActionListener interface.
     *
     * @param listenerClass - A class that implements the
     * TeamspeakActionListener interface.
     * @since 0.7
    ...
    /**
     * Remove the class from receiving Teamspeak events. This function also call
     * removeAllEvents(), if needed.
     *
     * @since 0.7
     */
     ...
     * @param channelID A channel ID, only needed for EVENT_MODE_CHANNEL. Use
     * any number for any other EVENT_MODE.
     * @return <code>true</code> if the event notify could be activated
     * successfully, <code>false</code> if not (check error with
     * getLastError()).
     * @since 0.7
     ...
     *
     * @return <code>true</code> if the events could be *      * deactivated, <code>false</code> if not (check error with
     * getLastError()).
     * @since 0.7
     * @see JTS3ServerQuery#getLastError()
     */
     ...
     * @return <code>true</code> if connection was *      * successful, <code>false</code> if not (check error with
     * getLastError()).
     * @see JTS3ServerQuery#getLastError()
     */
    ...
    /**
     * Open a query connection to the TS3 server, you can set a local IP and
     * port to bind the socket to it.
     *
     * @param ip IP or Host address to the TS3 server
     * @param queryport Query Port of the TS3 server
     * @param localIP Local IP address to bind the socket or <code>null</code>
     * to bind to default IP. localPort must be set to use localIP.
     * @param localPort Local Port to bind the socket, localIP must be set to
     * use localPort.
     * @return <code>true</code> if connection was *      * successful, <code>false</code> if not (check error with
     * getLastError()).
     * @since 1.0.4
     ...
     * @return <code>true</code> if login was successful, <code>false</code> if
     * an error occurred (check error with getLastError()).
     * @since 0.8
     * @see JTS3ServerQuery#getLastError()
     */
    ...
    /**
     * Login with an account. Optional you can change the display name for this
     * query connection.
     *
     * @deprecated This method also returns false, if login was successful, but
     * the display name is already in use. Use setDisplayName() after login
     * instead.
     * @param loginname Login name
     * @param password Login password
     * @param displayName Use this to set another display name for this server
     * query connection, at least 3 characters needed. Use <code>null</code> if
     * not wanted.
     * @return <code>true</code> if login was successful, <code>false</code> if
     * an error occurred (check error with getLastError()).
    ...
    /**
     * Change the display name on the Teamspeak 3 server of this query
     * connection. This name will be displayed on many actions like
     * kickClient(), moveClient(), pokeClient() and sendTextMessage().
     *
     * @param displayName A String with the new display name of this connection.
     * @return <code>true</code> if setting display name was *      * successful, <code>false</code> if an error occurred (check error
     * with getLastError()).
     * @since 0.8
     * @see JTS3ServerQuery#getLastError()
     */
     ...
     * if an error occurred (check error with getLastError()).
     * @see JTS3ServerQuery#getLastError()
     */
    ...
    /**
     * Select a virtual server to work with. This method allows to select the
     * virtual server by id or port.
     *
     * @param server A virtual server id or port
     * @param selectPort <code>true</code> if <code>server</code> is the virtual
     * server port, <code>false</code> if <code>server</code> is the virtual
     * server id.
     * @return <code>true</code> if selection was successful, <code>false</code>
     * if an error occurred (check error with getLastError()).
     * @since 0.9
    ...
    /**
     * Returns a String with the last error message of the last method. It also
     * contains the whole TS3 server error message, if an error
     * occurred.<br><br>
     * <b>Notice:</b><br>
     * The error message only contains an error message of the last used method
     * / function. That means you must call getLastError() directly after
     * another method / function, which had an error, to get the error message.
     *
     * @return A String with the last error message or <code>null</code> if no
     * error occurred.
     */
     ...
     * <b>Notice:</b><br>
     * The error code only contains an error code of the last used method /
     * function. That means you must call getLastErrorID() directly after
     * another method / function, which had an error, to get the error code.
     *
     ...
     * <b>Notice:</b><br>
     * The error code only contains an error code of the last used method /
     * function. That means you must call getLastErrorPermissionID() directly
     * after another method / function, which had an error, to get the error
     * code.
     *
     * @return An int with the permission id of the last error or -1 if the last
     * error did not contain a permission id.
     */
     ...
     * @param forceDelete <code>true</code> for a force channel delete (kicks
     * also clients out of it), <code>false</code> to delete only an empty
     * channel
     * @return <code>true</code> if channel delete was *      * successful, <code>false</code> if an error occurred (check error
     * with getLastError()).
     * @see JTS3ServerQuery#getLastError()
     */
     ...
     * @return <code>true</code> if client move was *      * successful, <code>false</code> if an error occurred (check error
     * with getLastError()).
     * @see JTS3ServerQuery#getLastError()
     */
     ...
     * @return <code>true</code> if client kick was *      * successful, <code>false</code> if an error occurred (check error
     * with getLastError()).
     * @see JTS3ServerQuery#getLastError()
     */
    ...
    /**
     * Returns the current client ID of the query connection. You need this
     * maybe to move the client or something else.
     *
    ...
    /**
     * Sends a text message to a client / channel / virtual server / global (all
     * virtual servers).<br><br>
     * <b>Notice:</b><br>
     * If you use a channel or virtual server id, which is not currently used by
     * this connection, sendTextMessage() work as follow:<br>
     * Switch to the channel or virtual server, sends the text message and
     * switch back to old channel or virtual server.<br><br>
     * If you want to send more messages to this channel or virtual server, just
     * use selectVirtualServer() or moveClient() first.
     *
     * @param targetID The client, channel or virtual server id. Use any number
     * for a global message.
     * @param targetMode A text message target mode constant
     * @param msg The message to be send
     * @return <code>true</code> if message was successfully *      * sent, <code>false</code> if an error occurred (check error with
     * getLastError()).
     ...
     * @param targetMode A text message target mode constant
     * @param msg The message to be send
     * @param channelPassword Channel password, is only needed for a text
     * message to channel. Use <code>null</code> if channel has no password or
     * not a channel text message.
     * @return <code>true</code> if message was successfully *      * sent, <code>false</code> if an error occurred (check error with
     * getLastError()).
     * @since 0.6
     * @see JTS3ServerQuery#TEXTMESSAGE_TARGET_CLIENT
    ...
    public boolean sendTextMessage(int targetID, int targetMode, String msg, String channelPassword)
    {
        ...
        try
        {
            ...
            {
                if (targetMode == TEXTMESSAGE_TARGET_CHANNEL)
                {
                    int oldChannel = -1;
                ...
                {
                    if (targetMode == TEXTMESSAGE_TARGET_CLIENT)
                    {
                    ...
                    {
                        if (targetMode == TEXTMESSAGE_TARGET_VIRTUALSERVER)
                        {
                            int oldServer = -1;
            ...
            if ((hmIn != null) && hmIn.get("id").equals("0"))
            {
            }
            else
            {
     ...
     * The returned HashMap can also contain a library error id and error
     * message, if the connection to the Teamspeak 3 Server got lost while
     * reading the response.<br><br>
     * <b>Important:</b><br>
     * Do not use the following commands here:<br>
     * <code>channeldelete</code><br>
     * <code>clientmove</code><br>
     * <code>use</code><br>
     * Please use deleteChannel(), moveClient() or selectVirtualServer()
     * instead!
     *
     * @param command Any TS3 telnet command, see TS3 documentation or use *      * the <code>help</code> command.
     * @return An HashMap with 3 keys: <code>id</code> (error *      * id), <code>msg</code> (error message) and <code>response</code>
     * (unformatted server response). Returns <code>null</code> if not connected
     * or no command given (check error with getLastError()).
     * @see JTS3ServerQuery#moveClient(int, int, String)
     * @see JTS3ServerQuery#selectVirtualServer(int)
     * @see JTS3ServerQuery#getLastError()
     */
    ...
    /**
     * Poke a client. This opens a message dialog at the selected Teamspeak 3
     * client with the given message.
     *
     * @param clientID The client ID, which should get the message.
     * @param msg The message for the message dialog.
     * @return <code>true</code> if clientpoke was *      * successful, <code>false</code> if not (check error with
     * getLastError()).
     * @since 0.4
     * @see JTS3ServerQuery#getLastError()
     */
     ...
     * @return <code>true</code> if complainadd was *      * successful, <code>false</code> if not (check error with
     * getLastError()).
     * @since 1.0
     * @see JTS3ServerQuery#getLastError()
     */
     ...
     *
     * @param clientDBID The client database ID, which should get a complain
     * removed.
     * @param deleteClientDBID Delete complains submitted from this client
     * database ID.
     * @return <code>true</code> if complaindel was *      * successful, <code>false</code> if not (check error with
     * getLastError()).
     * @since 1.0
     * @see JTS3ServerQuery#getLastError()
     */
    ...
    /**
     * Parse unformatted response from TS3 server, like from the doCommand
     * method.<br><br>
     * <b>Notice:</b><br>
     * Don't use this for help messages, since they are already formatted by the
     * TS3 server.
     *
     * @param rawData The unformatted TS3 server response
     * @return A Vector which contains a HashMap for each entry with the keys
     * given by the TS3 Server. Returns <code>null</code> if no rawData given.
     */
    public List<HashMap<String, String>> parseRawData(String rawData)
    {
        if (rawData == null)
        {
            return null;
     ...
     * <b>Notice:</b><br>
     * If you want server informations, the server will return informations only
     * about the current selected virtual server. To get informations about
     * another virtual server, just select first.
     *
     * @param infoMode An INFOMODE constant.
     * @param objectID A channel or client ID, use any number for server
     * informations.
     * @return A HashMap with the informations as key / value pairs like in the
     * TS3 server response. Returns <code>null</code> if any error occurred
     ...
     * If the permission ID was found, the HashMap will contain the following
     * keys:<br>
     * <code>permid</code> with the permission ID<br>
     * <code>permname</code> with the permission name<br>
     * <code>permdesc</code> with the permission description (may be empty, if
     * not exist)
     *
     * @param permID A permission ID
     * @return A HashMap with the information about the permission ID. *      * Returns <code>null</code> if any error occurred (check with
     * getLastError()) or if the permission ID was not found.
     * @see JTS3ServerQuery#getLastError()
     */
    public HashMap<String, String> getPermissionInfo(int permID)
    {
        resetLastError();
        List<HashMap<String, String>> permList = getList(LISTMODE_PERMISSIONLIST);
     ...
     * @return A Vector which contains a HashMap for each entry with the keys
     * given by the TS3 Server. Returns <code>null</code> if any error occurred
     * (check with getLastError()).
     ...
     * @see JTS3ServerQuery#getLastError()
     */
    public List<HashMap<String, String>> getPermissionList(int permListMode, int targetID)
    {
        resetLastError();
     ...
     * Returns log entries.
     *
     * @param linesCount How many log entries should be returned, has to be
     * between 1 and 100.
     * @param reverse Return lines in reverse order of the log (newest entry
     * first)?
     * @param masterlog Return lines from master instance log? Set to false to
     * get the log entries of the selected server!
     ...
     * @see JTS3ServerQuery#getLastError()
     */
    public List<HashMap<String, String>> getLogEntries(int linesCount, boolean reverse, boolean masterlog, int beginpos)
    {
        resetLastError();
    ...
    /**
     * Get a list from the TS3 server. Use LISTMODE constants to get the wanted
     * list.
     *
     ...
     * @see JTS3ServerQuery#getLastError()
     */
    public List<HashMap<String, String>> getList(int listMode)
    {
     ...
     * Get a list from the TS3 server. Use LISTMODE constants to get the wanted
     * list.<br><br>
     * This method allows to pass many arguments separated with comma, see
     * LISTMODE comments for possible arguments.
     *
     * @param listMode Use a LISTMODE constant
     * @param arguments A comma separated list of arguments or a single argument
     * for the LISTMODE. Or just <code>null</code> if no arguments needed.
     * @return A Vector which contains a HashMap for each entry with the keys
     * given by the TS3 Server. Returns <code>null</code> if any error occurred
     * (check with getLastError()).
     * @see JTS3ServerQuery#LISTMODE_BANLIST
     ...
     * @see JTS3ServerQuery#getLastError()
     */
    public List<HashMap<String, String>> getList(int listMode, String arguments)
    {
        resetLastError();
    ...
    private List<HashMap<String, String>> getList(String command)
    {
        if (!isConnected())
        {
    ...
    private HashMap<String, String> readIncoming()
    {
        ...
        while (true)
        {
            ...
            catch (SocketTimeoutException | SocketException e)
            {
                ...
                closeTS3Connection();
                hmIn.put("id", "-2");
                hmIn.put("msg", "Closed TS3 Connection, Exception readIncoming(): " + e.toString());
                return hmIn;
            }
            catch (Exception e)
            {
    ...
    /**
     * Escape all special characters for the TS3 server.<br>Use this for all
     * Strings you use as value while using doCommand()!<br><br>
     * <b>Important:</b><br>
     * Almost all functions in this library do this already if needed. You only
     * need this if you want to send an own command with doCommand().
     *
    ...
    /**
     * Convert escaped characters to normal characters.<br>Use this for received
     * String values after using doCommand()<br><br>
     * <b>Important:</b><br>
     * Almost all functions in this library do this already if needed. You only
     * need this if you want to read the server response after using doCommand()
     * without using parseRawData().
     *
    ...
    private HashMap<String, String> parseLine(String line)
    {
        ...
        HashMap<String, String> retValue;
    ...
    private String getCommand(int mode, int listType)
    {
        if (listType == 1)
        {
            ...
            {
                if (mode == LISTMODE_CLIENTDBLIST)
                {
                    return "clientdblist";
                }
                else
                {
                    if (mode == LISTMODE_CLIENTLIST)
                    {
                        return "clientlist";
                    }
                    else
                    {
                        if (mode == LISTMODE_PERMISSIONLIST)
                        {
                            return "permissionlist";
                        }
                        else
                        {
                            if (mode == LISTMODE_SERVERGROUPLIST)
                            {
                                return "servergrouplist";
                            }
                            else
                            {
                                if (mode == LISTMODE_SERVERLIST)
                                {
                                    return "serverlist";
                                }
                                else
                                {
                                    if (mode == LISTMODE_BANLIST)
                                    {
                                        return "banlist";
                                    }
                                    else
                                    {
                                        if (mode == LISTMODE_COMPLAINLIST)
                                        {
                                            return "complainlist";
                                        }
                                    }
        ...
        else
        {
            if (listType == 2)
            {
                ...
                {
                    if (mode == INFOMODE_CHANNELINFO)
                    {
                        return "channelinfo cid=";
                    }
                    else
                    {
                        if (mode == INFOMODE_CLIENTINFO)
                        {
            ...
            else
            {
                if (listType == 3)
                {
                    ...
                    {
                        if (mode == PERMLISTMODE_CLIENT)
                        {
                            return "clientpermlist cldbid=";
                        }
                        else
                        {
                            if (mode == PERMLISTMODE_SERVERGROUP)
                            {
